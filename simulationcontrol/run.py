...
def save_output(
    base_configuration, benchmark, console_output, cpistack, started, ended
):
    ...
    with open(os.path.join(directory, "cpi-stack.txt"), "wb") as f:
        f.write(cpistack)
    for f in ("sim.cfg", "sim.info", "sim.out", "cpi-stack.png", "sim.stats.sqlite3"):
    ...
    for f in (
        ...
        with open(os.path.join(BENCHMARKS, f), "rb") as f_in, gzip.open(
            "{}.gz".format(os.path.join(directory, f)), "wb"
        ) as f_out:
...
def run(base_configuration, benchmark, ignore_error=False):
    ...
    args = "-n {number_cores} -c {config} --benchmarks={benchmark} --no-roi --sim-end=last -senergystats:{periodic} -speriodic-power:{periodic}{script}{benchmark_options}".format(
    ...
    try:
        cpistack = subprocess.check_output(
            ["python", os.path.join(SNIPER_BASE, "tools/cpistack.py")], cwd=BENCHMARKS
        )
    except:
        if ignore_error:
            cpistack = b""
...
def get_threads():
    threads = {
        ...
        "splash2-water.sp": [
            ...
            0,
            16,
        ],  # other parallelism values run but are suboptimal -> don't allow in the first place
    }
...
def get_workload(
    benchmark, cores, parallelism=None, number_tasks=None, input_set="small"
):
    if parallelism is not None:
        number_tasks = math.floor(cores / parallelism)
        return get_workload(
            benchmark, cores, number_tasks=number_tasks, input_set=input_set
        )
...
def test_static_power():
    run(
        ["4.0GHz", "testStaticPower", "slowDVFS"],
...
def ondemand_demo():
    run(
        ["{:.1f}GHz".format(4), "ondemand", "fastDVFS"],
        get_instance("parsec-blackscholes", 3, input_set="simsmall"),
    )
...
def coldest_core_multiprogram_test():
    benchmark_set = [
        'parsec-x264',
        'parsec-x264',
    ]
