...
def plot_results(data, x_label, y_label, plot_name="", legend=False):
    ...
    for bench, values in data.items():
        ...
        ax.plot(
            ...
            y,
            label=bench,
            color=colors[bench][0],
            linestyle=colors[bench][1],
            marker="o",
        )
    ...
    print("saving plots/" + x_label + "_" + plot_name + "_" + y_label.replace("°", "") + ".png...")
    plt.savefig(
        "plots/" + x_label + "_" + plot_name + "_" + y_label.replace("°", "") + ".png",
        bbox_inches="tight",
        pad_inches=0.1,
    )
    # close the plt
...
def make_cranky_cores_plot(header, filterstring):
    headers, data = parse_results_returner()
    header_index = headers.index(header)
    name_index = headers.index("name")
    filter_index = headers.index("filter")
...
def make_freq_plot(header, filterstring, cores):
    ...
    filtered_data = [
        d
        for d in data
        if "+".join(d[filter_index].split("+")[1:]) == filterstring
        and d[cores_index] == int(cores)
    ]
...
def make_multiprogram_plot(header):
    ...
    for d in filtered_data:
        if str(d[freq_index]) + "GHz" not in data:
            data[str(d[freq_index]) + "GHz"] = [
                (configs.index(d[config_index]) + 1, d[header_index])
            ]
        else:
            data[str(d[freq_index]) + "GHz"].append(
                (configs.index(d[config_index]) + 1, d[header_index])
            )
...
def make_coldest_core_plot(header):
    ...
    filtered_data = [
        d for d in data if d[filter_index] == "2.0GHz+maxFreq+slowDVFS+coldestCore"
    ]
    ...
    filtered_data_no_coldest = [
        d
        for d in data
        if d[filter_index] == "4.0GHz+maxFreq+slowDVFS"
        and d[config_index] in filtered_configs
    ]
    ...
    for d in filtered_data:
        ...
        corresponding_data = [
            x for x in filtered_data_no_coldest if x[config_index] == d[config_index]
        ][0]
...
def make_craknfunkmultiprogram_plot(header):
    headers, data = parse_results_returner()
    header_index = headers.index(header)
    name_index = headers.index("name")
    filter_index = headers.index("filter")
    ...
    funky_string = "funky+fastDVFS+2arrivals"
    cranky_string = "maxFreq+fastDVFS+cranky+2arrivals"

    # get all the data
    filtered_cranky_data = [d for d in data if "+".join(d[filter_index].split("+")[1:])  == cranky_string]
    filtered_funky_data = [d for d in data if "+".join(d[filter_index].split("+")[1:])  == funky_string]

    data = {"funky": [], "cranky": []}
    for d in filtered_cranky_data:
        data["cranky"] += [
            (d[freq_index], d[header_index]),
        ]
    
    for d in filtered_funky_data:
        data["funky"] += [
            (d[freq_index], d[header_index]),
        ]

    plot_results(data, "freq (GHz)", header, "Cranky vs Funky multiprogram")



def make_funky_vs_cranky_plot(header, ghz):
    headers, data = parse_results_returner()
    header_index = headers.index(header)
    name_index = headers.index("name")
    filter_index = headers.index("filter")
    ...
    funky_string = ghz + ".0GHz+funky+fastDVFS"
    cranky_string = ghz + ".0GHz+maxFreq+fastDVFS+cranky"

    # get all the data
    filtered_cranky_data = [d for d in data if d[filter_index] == cranky_string]
    filtered_funky_data = [d for d in data if d[filter_index] == funky_string]

    data = {"funky": [], "cranky": []}
    for d in filtered_cranky_data:
        # find the corresponding data without coldest core
        corresponding_data = [
            x for x in filtered_funky_data if x[name_index] == d[name_index]
        ]
        if len(corresponding_data) == 0:
            continue
        corresponding_data = corresponding_data[0]

        data["funky"] += [
            (corresponding_data[name_index], corresponding_data[header_index]),
        ]

        data["cranky"] += [
            (d[name_index], d[header_index]),
        ]
    ...
    plot_results(
        ...
        "simulation",
        header,
        "Cranky vs Funky " + ghz + ".0GHz+fastDVFS",
        legend=True,
    )

make_funky_vs_cranky_plot("avg resp time (ns)", "2")
make_funky_vs_cranky_plot("avg power (W)", "2")
make_funky_vs_cranky_plot("energy (J)", "2")
make_funky_vs_cranky_plot("peak temperature (C)", "2")
make_funky_vs_cranky_plot("sim. time (ns)", "2")
make_funky_vs_cranky_plot("avg resp time (ns)", "3")
make_funky_vs_cranky_plot("avg power (W)", "3")
make_funky_vs_cranky_plot("energy (J)", "3")
make_funky_vs_cranky_plot("peak temperature (C)", "3")
make_funky_vs_cranky_plot("sim. time (ns)", "3")
make_funky_vs_cranky_plot("avg resp time (ns)", "4")
make_funky_vs_cranky_plot("avg power (W)", "4")
make_funky_vs_cranky_plot("energy (J)", "4")
make_funky_vs_cranky_plot("peak temperature (C)", "4")
make_funky_vs_cranky_plot("sim. time (ns)", "4")

make_craknfunkmultiprogram_plot("avg resp time (ns)")
make_craknfunkmultiprogram_plot("avg power (W)")
make_craknfunkmultiprogram_plot("energy (J)")
make_craknfunkmultiprogram_plot("peak temperature (C)")
make_craknfunkmultiprogram_plot("sim. time (ns)")
...
# make_cores_plot('avg resp time (ns)', '1.0GHz+maxFreq+fastDVFS')
# make_cores_plot('avg resp time (ns)', '2.0GHz+maxFreq+fastDVFS')
# make_cores_plot('avg resp time (ns)', '3.0GHz+maxFreq+fastDVFS')
# make_cores_plot('avg resp time (ns)', '4.0GHz+maxFreq+fastDVFS')
# make_cores_plot('avg power (W)', '1.0GHz+maxFreq+fastDVFS')
# make_cores_plot('avg power (W)', '2.0GHz+maxFreq+fastDVFS')
# make_cores_plot('avg power (W)', '3.0GHz+maxFreq+fastDVFS')
# make_cores_plot('avg power (W)', '4.0GHz+maxFreq+fastDVFS')
# make_cores_plot('energy (J)', '1.0GHz+maxFreq+fastDVFS')
# make_cores_plot('energy (J)', '2.0GHz+maxFreq+fastDVFS')
# make_cores_plot('energy (J)', '3.0GHz+maxFreq+fastDVFS')
# make_cores_plot('energy (J)', '4.0GHz+maxFreq+fastDVFS')
# make_cores_plot('peak temperature (C)', '1.0GHz+maxFreq+fastDVFS')
# make_cores_plot('peak temperature (C)', '2.0GHz+maxFreq+fastDVFS')
# make_cores_plot('peak temperature (C)', '3.0GHz+maxFreq+fastDVFS')
# make_cores_plot('peak temperature (C)', '4.0GHz+maxFreq+fastDVFS')
# make_cores_plot('sim. time (ns)', '1.0GHz+maxFreq+fastDVFS')
# make_cores_plot('sim. time (ns)', '2.0GHz+maxFreq+fastDVFS')
# make_cores_plot('sim. time (ns)', '3.0GHz+maxFreq+fastDVFS')
# make_cores_plot('sim. time (ns)', '4.0GHz+maxFreq+fastDVFS')
...
# make_cores_plot("avg resp time (ns)", "2.0GHz+maxFreq+fastDVFS+cranky")
# make_cores_plot("avg power (W)", "2.0GHz+maxFreq+fastDVFS+cranky")
# make_cores_plot("energy (J)", "2.0GHz+maxFreq+fastDVFS+cranky")
# make_cores_plot("peak temperature (C)", "2.0GHz+maxFreq+fastDVFS+cranky")
# make_cores_plot("sim. time (ns)", "2.0GHz+maxFreq+fastDVFS+cranky")
#
# make_cores_plot("avg resp time (ns)", "3.0GHz+maxFreq+fastDVFS+cranky")
# make_cores_plot("avg power (W)", "3.0GHz+maxFreq+fastDVFS+cranky")
# make_cores_plot("energy (J)", "3.0GHz+maxFreq+fastDVFS+cranky")
# make_cores_plot("peak temperature (C)", "3.0GHz+maxFreq+fastDVFS+cranky")
# make_cores_plot("sim. time (ns)", "3.0GHz+maxFreq+fastDVFS+cranky")
#
# make_cores_plot("avg resp time (ns)", "4.0GHz+maxFreq+fastDVFS+cranky")
# make_cores_plot("avg power (W)", "4.0GHz+maxFreq+fastDVFS+cranky")
# make_cores_plot("energy (J)", "4.0GHz+maxFreq+fastDVFS+cranky")
# make_cores_plot("peak temperature (C)", "4.0GHz+maxFreq+fastDVFS+cranky")
# make_cores_plot("sim. time (ns)", "4.0GHz+maxFreq+fastDVFS+cranky")

make_freq_plot("avg resp time (ns)", "maxFreq+fastDVFS+cranky", "4")
make_freq_plot("avg power (W)", "maxFreq+fastDVFS+cranky", "4")
make_freq_plot("energy (J)", "maxFreq+fastDVFS+cranky", "4")
make_freq_plot("peak temperature (C)", "maxFreq+fastDVFS+cranky", "4")
make_freq_plot("sim. time (ns)", "maxFreq+fastDVFS+cranky", "4")
